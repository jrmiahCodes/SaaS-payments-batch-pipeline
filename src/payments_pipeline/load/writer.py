...
class BronzeWriter:
    def __init__(self, settings: Settings):
        ...
        self._s3 = boto3.client("s3", region_name=settings.aws_region) if boto3 and settings.pipeline_env == "AWS" else None
    ...
    def write_bronze_jsonl(
        ...
        *,
        ...
        if deterministic_order:
            records = sorted(records, key=lambda r: str(r.get("data", {}).get("id", r.get("id", ""))))
        ...
        if write_sidecar and paths:
            ...
            sidecar_rel = bronze_relative_path(entity=entity, dt=dt, run_id=run_id, part=0).replace("part-00000.jsonl", "_metadata.json")
